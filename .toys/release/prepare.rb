...
def prepare_release gem_name
  ...
  release_info = ReleaseInfo.new gem_name, gem_directory, override_version, self
...
def modify_changelog release_info
  path = "CHANGELOG.md"
  content = ::File.read path
  content.sub!(/### (v\d+\.\d+\.\d+ \/ \d\d\d\d-\d\d-\d\d)/, "#{release_info.full_changelog}### \\1")
  ::File.open(path, "w") { |file| file.write content }
end
...
def create_release_pr release_info
  pr_body = <<~STR
    ...
    Feel free to edit the changlog if desired.
...
class ReleaseInfo
  def initialize gem_name, gem_directory, override_version, context
    @context = context
    @gem_name = gem_name
  ...
  def analyze_messages
    shas =
      if @last_version
        @context.capture(["git", "log", "#{@gem_name}/v#{@last_version}^..HEAD", "--format=%H"]).split("\n").reverse
      else
        []
      end
  ...
  def build_commit_info
    @release_commit_title = "release: Release cloud_events #{@new_version}"
