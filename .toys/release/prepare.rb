...
def verify_no_duplicate_release gem_name
  prs = find_release_prs repo_path, gem_name: gem_name
  unless prs.empty?
...
def create_release_pr release_info
  pr_body = <<~STR
    ...
    This pull request prepares a new release of #{release_info.gem_name}. To \
    confirm the release, merge, ensuring the #{release_pending_label.inspect} \
    label is set.

    An initial changelog has been constructed from \
    [conventional commit](https://conventionalcommits.org) messages, and is \
    provided in this pull request (and is also quoted below). Feel free to \
    edit the changelog and/or release version before merging. Note that the \
    changelog header format must remain the same, and must match the release \
    version, or the release will not succeed.
  ...
  response = capture ["gh", "api", "repos/#{repo_path}/pulls", "--input", "-",
                      "-H", "Accept: application/vnd.github.v3+json"],
                     in: [:string, body]
  pr = ::JSON.parse(response)
  update_release_pr repo_path, pr["number"], label: release_pending_label, cur_pr: pr
end
