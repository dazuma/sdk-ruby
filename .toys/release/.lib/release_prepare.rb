...
class ReleasePrepare
  ...
  class Instance
    ...
    def determine_last_version
      @last_version = @utils.capture(["git", "tag", "-l"])
        .split("\n")
        .map do |tag|
    ...
    def create_release_pr
      pr_body = <<~STR
        ...
        This pull request prepares a new release of #{@gem_name}. To \
        confirm the release, merge, ensuring the #{@utils.release_pending_label.inspect} \
        label is set.
