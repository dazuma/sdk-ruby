...
class ReleaseUtils
  ...
  def update_release_pr pr_number, label: nil, message: nil, state: nil, cur_pr: nil
    ...
    if state && (!cur_pr || cur_pr["state"] != "closed")
  ...
  def verify_changelog_content gem_name, gem_vers
  ...
  def verify_github_checks ref: nil
    ...
    unless result.success?
      error "Failed to obtain GitHub check results for #{ref}"
      return
    end
    ...
    unless checks.size == results["total_count"]
      error "GitHub check count mismatch for #{ref}"
    end
    ok = true
