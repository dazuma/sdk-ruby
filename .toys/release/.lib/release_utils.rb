...
class ReleaseUtils # rubocop:disable Metrics/ClassLength
  ...
  attr_reader :perform_workflow_name
  ...
  def report_release_error pr_info, content
    ...
    body = <<~STR
      ...
      Commit: https://github.com/#{repo_path}/commit/#{merge_sha}
    ...
    response = capture ["gh", "api", "repos/#{repo_path}/issues", "--input", "-",
                        "-H", "Accept: application/vnd.github.v3+json"],
                       in: [:string, input]
  ...
  def load_release_info file_path
    ...
    @perform_workflow_name = info["perform_workflow_name"] || "release-perform.yml"
