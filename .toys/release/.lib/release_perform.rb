...
class ReleasePerform
  ...
  def initial_setup
    ...
    setup_rubygems_api_key if @enable_gem
  ...
  class Instance
    ...
    def perform only: nil
      ...
      build_gem if !only || only == "gem"
      if @docs_builder && (!only || only == "docs")
        build_docs
        set_default_docs_version if @gem_version =~ /^\d+\.\d+\.\d+$/
      end

      create_github_release @changelog_content if !only || only == "github-release"
      push_gem if !only || only == "gem"
      push_docs if @docs_builder && (!only || only == "docs")
    ...
    def create_github_release content = nil
      if @dry_run
        @utils.logger.info "DRY RUN: Created github release of #{@gem_name} #{@gem_version}"
        return
      end
    ...
    def build_gem
      logger.info "Building #{@gem_name} #{@gem_version} gem ..."
    ...
    def push_docs
      ...
      ::Dir.chdir @gh_pages_dir do
        exec ["git", "add", "."]
        exec ["git", "commit", "-m", "Generate yardocs for #{@gem_name} #{@gem_version}"]
        if @dry_run
