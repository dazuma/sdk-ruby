...
required_arg :gem_name do
  desc "Name of the gem to release"
end
required_arg :gem_version do
  desc "Version of the gem to release"
end
...
def run
  ...
  pr_info = find_release_pr utils
  if pr_info
    utils.on_error { |content| utils.report_release_error pr_info, content }
  end
  performer = create_performer utils
  confirm_release utils
  performer.perform only: only
  if pr_info
    utils.clear_error_proc
    mark_release_pr_as_completed pr_info, utils
  end
  nil
end

def find_release_pr utils
  if release_pr
...
def create_performer utils
  ...
  performer = ReleasePerform.new utils,
                                 ...
                                 dry_run:          dry_run
  performer.instance gem_name, gem_version
end

def confirm_release utils
  return if yes
...
def mark_release_pr_as_completed gem_name, gem_version, pull, utils
  ...
  logger.info "Marking release PR ##{pr_number} complete ..."
